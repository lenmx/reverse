...
var (
	...
	defaultGolangTemplate = fmt.Sprintf(`{{$reverseConfig := .ReverseConfig}}
...
{{$table := .}}{{range .ColumnsSeq}}{{$col := $table.GetColumn .}}	{{ColumnMapper $col.Name}}	{{Type $col}} %s{{Tag $table $col $reverseConfig}}%s
{{end}}
}
{{end}}
`, "`", "`")
	defaultGolangTemplateTable = fmt.Sprintf(`{{$reverseConfig := .ReverseConfig}}
package {{GenPkgname $reverseConfig}}
...
//func tag(table *schemas.Table, col *schemas.Column, reverseConfig *config.ReverseTarget) template.HTML {
func tag(table *schemas.Table, col *schemas.Column, reverseConfig *common.ReverseTarget) template.HTML {
	...
	//if jsonTag {
	//	res = append(res, fmt.Sprintf("json:\"%s\"", col.FieldName))
	//}
	...
	if len(res) > 0 {
		resMerge = append(resMerge, fmt.Sprintf("xorm:\"%s\"", strings.Join(res, " ")))
	}

	if (reverseConfig.JsonTag) {
		mapper := common.ConvertString2Mapper(reverseConfig.JsonTagMapper)
		fieldName := mapper.Obj2Table(col.FieldName)
		resMerge = append(resMerge, fmt.Sprintf("json:\"%s\"", fieldName))
	}

	if len(resMerge) > 0 {
		return template.HTML(fmt.Sprintf(`%s`, strings.Join(resMerge, " ")))
	}
...
func genPkgname(reverseConfig *common.ReverseTarget) (pkgname string) {
	index := strings.LastIndex(reverseConfig.OutputDir, "/")
	if index == -1 {
		pkgname = reverseConfig.OutputDir
		return
	}

	pkgname = reverseConfig.OutputDir[index+1:]
	return
}
